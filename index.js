...
const db = new Database('./data/sentinela.db');
...
const insertShop = db.prepare(`INSERT OR IGNORE INTO shop (nome, descricao, preco, emoji) VALUES (@nome, @descricao, @preco, @emoji)`);
...
function addAura(id, qt) {
  db.prepare('UPDATE usuarios SET aura = aura + ?, aura_total = aura_total + ? WHERE id = ?').run(qt, qt > 0 ? qt : 0, id);
}
function removeAura(id, qt) {
  const u = getUser(id);
  const r = Math.min(qt, u.aura);
  db.prepare('UPDATE usuarios SET aura = aura - ? WHERE id = ?').run(r, id);
  return r;
}
function addXP(id, xp) {
  const u = getUser(id);
  let novoXP = u.xp + xp, novoNivel = u.nivel;
...
function getRanking(limit = 10) { return db.prepare('SELECT * FROM usuarios ORDER BY aura DESC LIMIT ?').all(limit); }
function getInventario(uid) { return db.prepare('SELECT * FROM inventario WHERE user_id = ?').all(uid); }
function addItem(uid, item) {
  const ex = db.prepare('SELECT * FROM inventario WHERE user_id = ? AND item = ?').get(uid, item);
  if (ex) db.prepare('UPDATE inventario SET quantidade = quantidade + 1 WHERE user_id = ? AND item = ?').run(uid, item);
  else db.prepare('INSERT INTO inventario (user_id, item) VALUES (?, ?)').run(uid, item);
}
function removeItem(uid, item) {
  const ex = db.prepare('SELECT * FROM inventario WHERE user_id = ? AND item = ?').get(uid, item);
  if (!ex) return false;
  if (ex.quantidade <= 1) db.prepare('DELETE FROM inventario WHERE user_id = ? AND item = ?').run(uid, item);
  else db.prepare('UPDATE inventario SET quantidade = quantidade - 1 WHERE user_id = ? AND item = ?').run(uid, item);
  return true;
}
function hasItem(uid, item) { return !!db.prepare('SELECT * FROM inventario WHERE user_id = ? AND item = ?').get(uid, item); }
...
commands.push({
  ...
  async execute(interaction) {
    ...
    const item = db.prepare('SELECT * FROM shop WHERE LOWER(nome) = LOWER(?) AND ativo = 1').get(nome);
...
commands.push({
  ...
  async execute(interaction) {
    ...
    if (!inv.length) embed.setDescription('Seu inventário está vazio!\nCompre itens na **/loja**.');
      ...
      const linhas = inv.map(i => { const s = db.prepare('SELECT emoji FROM shop WHERE nome = ?').get(i.item); return `${s?.emoji || '📦'} **${i.item}** x${i.quantidade}`; }).join('\n');
  ...
  async execute(interaction) {
    ...
    collector.on('collect', async i => {
      ...
      if (i.customId === 'casar_aceitar') {
        ...
        db.prepare('UPDATE usuarios SET casado_com = ?, data_casamento = ? WHERE id = ?').run(alvo.id, data, interaction.user.id);
        db.prepare('UPDATE usuarios SET casado_com = ?, data_casamento = ? WHERE id = ?').run(interaction.user.id, data, alvo.id);
...
commands.push({
  ...
  async execute(interaction) {
    ...
    collector.on('collect', async i => {
      ...
      db.prepare('UPDATE usuarios SET wins = wins + 1 WHERE id = ?').run(venc.id);
...
commands.push({
  ...
  async execute(interaction) {
    ...
    db.prepare(`INSERT INTO giveaways (message_id, channel_id, guild_id, premio, aura, host_id, vencedores, termina_em) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(msg.id, interaction.channel.id, interaction.guild.id, premio, auraPremi, interaction.user.id, vencedores, terminaEm.toISOString());
    ...
    collector.on('collect', async i => {
      ...
      if (!gaw) return;
      if (db.prepare('SELECT * FROM giveaway_participantes WHERE giveaway_id = ? AND user_id = ?').get(gaw.id, i.user.id)) return i.reply({ content: '🎟️ Você já está participando!', ephemeral: true });
    ...
    collector.on('end', async () => {
      ...
      const parts = db.prepare('SELECT user_id FROM giveaway_participantes WHERE giveaway_id = ?').all(gaw.id);
...
commands.push({
  ...
  async execute(interaction) {
    ...
    const s = db.prepare('SELECT emoji FROM shop WHERE LOWER(nome) = LOWER(?)').get(item);
